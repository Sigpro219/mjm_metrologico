...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside 
        className={`fixed md:static inset-y-0 left-0 z-40 w-[230px] flex flex-col h-screen shrink-0 border-r border-[#1e293b] text-white transition-transform duration-300 ease-in-out
          ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
        `}
        style={{ backgroundColor: 'var(--color-secondary, #0B1437)' }}
      >
        {/* Mobile Close Button */}
        <button 
          onClick={closeMobile} 
          className="md:hidden absolute top-4 right-4 text-white/50 hover:text-white p-2 rounded-full hover:bg-white/10 transition-colors"
        >
        ...
        <div className="py-6 flex flex-col items-center border-b border-white/5 shrink-0 bg-white/5 gap-3">
          <div className="flex flex-col items-center gap-3 w-full justify-center px-4 text-center">
            {logoUrl ? (
              <div className="bg-white/10 p-3 rounded-2xl w-full flex justify-center shadow-inner mt-4 md:mt-0">
                <Image
                  src={logoUrl}
                  alt={brandName}
                  width={180}
              ...
              <div className="flex items-center justify-center gap-3 text-white font-bold text-xl tracking-wide w-full mb-1 mt-4 md:mt-0">
